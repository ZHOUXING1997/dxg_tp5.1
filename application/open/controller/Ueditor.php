...
class Ueditor extends AdminBase
{
    ...
    private function _get_remote_image()
    {
        ...
        $strSavePath       = Env::get('root_path') . 'public' . DS . "ueditor" . DS . $date . DS;
        //远程抓取图片配置
        $config = [
        ...
        $storage_setting = $this->getCmfSettings('storage');
    ...
    public function getCmfSettings ($key = "") {
        $cmfSettings = cache("cmf_settings");
        if (empty($cmfSettings)) {
            $objResult  = app()->model('open/Option')->where(["option_name" => 'cmf_settings'])->find();
    ...
    private function ueditorUpload($fileType = 'image')
    {
        ...
        if ($result === false) {
            ...
            return json_encode([
                'state'    => 'SUCCESS',
                'url'      => handle_image_url($result['url']),
                'title'    => $result['name'],
    ...
    private function ueditorConfig()
    {
        ...
        $config_text    = preg_replace("/\/\*[\s\S]+?\*\//", "", file_get_contents("./static/ueditor/php/config.json"));
...
//
    ...
    public function imageManager()
    {
        ...
        foreach ($files as $file) {
            $str .= Env::get('root_path') . '/' . $file . "ue_separate_ue";
        }
        echo $str;
    }
